...
可直接遊玩的 Godot 4.6 俯視角 2D 動作戰鬥／軍隊養成遊戲，同時提供原生桌面版與已匯出的瀏覽器版。世界依固定種子以 Chunk 持續生成；玩家探索草原與稀疏分布的蟒蛇 Boss 巢穴、招募 16 種士兵、挑戰三種世界 Boss，並攻略 20／30／35／40／45／50 級科技城堡取得被動收入。角色、環境、UI、粒子特效與音效皆由程式產生。
...
- `C`：角色能力面板；點擊 `+` 消耗技能點升級。
- `K`：開啟「士兵永久強化」；花費大量金幣購買兵種能力，僅之後招募或復活的該兵種會取得新快照，現有士兵不變。
...
- `技能`：施放等級 10 解鎖的職業技能。
- `說明`：顯示／隱藏觸控教學。
- `地圖`、`能力`、`兵強`、`招募`、`軍令`、`暫停`：開啟對應功能；「兵強」完整支援兵種切換、基礎／特殊分頁、購買與關閉。
- 被蟒皇纏縛時，連點右側攻擊區掙脫。
...
公開倉庫根目錄為 GitHub Pages 編譯成品，Godot 4.6 原始碼放在 `source/`：<https://github.com/zianyu503-art/infinite-legion-wildland-expedition>。
...
成功時終端會顯示 `SELF_TEST_PASS`。目前共有 **249** 項測試，涵蓋無限地圖、國家戰線與城池吞併、三種 Boss、16 兵種、57 種永久特殊能力、強化價格與未來招募快照、Schema 7→8 存檔遷移、敵軍特殊強化、通知開關、作弊碼隔離、中英雙語介面，以及桌面與觸控版兵種強化商店、方向箭頭、攻擊搖桿、招募、軍令、暫停與全螢幕虛擬按鍵。
